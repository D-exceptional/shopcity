...
(async function ($) {
  ...
  $(".view-link").on("click", async function (e) {
    ...
    if ($(this).hasClass("inbox-previous")) {
      if (inboxTracker === 1) {
        ...
        inboxTracker--;
        url = `/mail/inbox/${inboxTracker}`;
        payload = { total: limit };
        
        await fetchMails(url, payload);
      }
    }
  ...
  // ----------------------------------------------------
  async function fetchMails(endpoint, payload) {
    try {
      ...
      const html = `
        <tr class='mail-list content-row' data-id='${mail_id}'>
          ...
          <td class="mailbox-name">
            <a href="/admin/mail/${mail_id}">${mail_sender}</a>
          </td>

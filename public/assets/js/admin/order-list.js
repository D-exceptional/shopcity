  ...
  */
  ...
  $(".btn-load").on("click", async function () {
    ...
    async function fetchOrders(payload) {
      const filter   = ["Pending", "Completed", "Cancelled"].includes(status) ? status : 'All';
      const endpoint = filter === 'All'
        ? `/order/page/${payload.page}`
        : `/order/status/${status}/page/${payload.page}`;

...
// Import Validations
// ----------------------------------------------------
import { validateInput, extractAmount } from "../../js/shared/utils/index.js";
...
(function ($) {
  ...
  // -------------------------------------------------
  let storeId = null; // Store ID that the coupon applies to
  ...
  if (allSame) {
    storeId = storeIds[0]; // Save the single store ID
  ...
  // -------------------------------------------------
  // Apply Coupon (GET /store/coupon/check)
  // -------------------------------------------------
  $(document).on("click", ".btn-coupon", async function () {
    const couponCode = validateInput($(".coupon-code"));
    ...
    try {
      const result = await makeRequest(
        `/store/${storeId}/coupon/${couponCode}`,
        "GET",
        {},
      );

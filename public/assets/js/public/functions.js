...
import {
  ...
  logout,
  redirect,
} from "../../js/shared/modules/index.js";
...
(function ($) {
  ...
  $(document).on("click", ".start-search", function () {
    ...
    // Proceed if valid
    const resultPage = `/product/${searchValue}/search`;
  ...
  $(document).on("change", ".main-category", function () {
    ...
    const resultPage = `/products/category/${encodeURIComponent(
      category
    )}/page/1`;
  ...
  $(document).on(
    ...
    function () {
      ...
      const linkPath = `/product/${encodeURIComponent(
        productId
      )}`;
  ...
  $(document).on("click", "a.product-share", function (e) {
    ...
    */
    ...
    shareContent({
      title: productName,
      text: "Check out this awesome product on ShopCity marketplace!",

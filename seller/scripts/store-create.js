...
(async function ($) {
  ...
  $(".form-type").on("change", function () {
    const storeType = $(this).val().trim();

    if (!storeType) {
      displayMessage("Select your store type to proceed", "info");
      return;
    } 
    ...
    displayMessage(
      `${
        storeType === "Regular"
          ? "This means you'll be running a conventional online store and have the products at hand"
          : "This means you'll be running a dropshipping store and don't have the products at hand"
      }`,
      "info"
    );
  });

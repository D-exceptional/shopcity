<?php
    ...
    $storeId = isset($_GET['id']) && is_numeric($_GET['id']) ? intval($_GET['id']) : null;
    $view    = isset($_GET['view']) && is_string($_GET['view']) ? $_GET['view'] : 'category';
    $param   = isset($_GET['data']) && is_string($_GET['data']) ? $_GET['data'] : null;

    // Store Products
    $storeProducts = ($view === 'category') ? $productModel->findByStoreCategory($storeId, $param, $page) : $productModel->findByStoreColor($storeId, $param, $page);

    // Fetch Categories & Colors
    ...
    $baseUrl     = "product-store?id={$storeId}&view={$view}&data={$param}";
?>
...
<body>
    ...
    <div class="container-fluid page-header py-5">
        ...
        <p class="text-white mb-3 text-center"><?= $storeDetails['store_description'] ?></p>

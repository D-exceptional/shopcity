...
<div class="wrapper">
  ...
  <div class="content-wrapper" data-role="<?= $role ?>">
    <!-- Content Header (Page header) -->
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6 header-count">
            <h1><b><?= $role . 's' ?> (<?= $numberManager->format($userList['total']) ?>)</b></h1>
          </div>
          <div class="col-sm-6">
              ...
              <?php if (!empty($userList['users'])): ?>
                ...
                <?php foreach ($userList['users'] as $user): ?>
                  ...
                  <tr class='content-row' data-id='<?=$user['user_id'] ?>'>
                    <td>#</td>
                    <td>
                      <img src='<?= ($user['avatar'] !== 'None') ? $user['avatar'] : asset('img/avatar.jpg') ?>' 
                ...
                <?php else: ?>
                  <!-- No User Available -->
                  <td colspan="12" class='text-center'>No<?= strtolower($role) . 's ' ?> available</td> 
  ...
  <!-- Custom Scripts -->
  <script src="<?= asset_versioned('js/admin/user-list.js'); ?>" type="module"></script>

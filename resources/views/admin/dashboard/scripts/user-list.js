...
(async function ($) {
  ...
  $(".btn-load").on("click", async function () {
    ...
    await fetchUsers({ page: offset, total: limit });
  });
  ...
  $(document).on("click", ".action button", async function () {
    ...
    if ($(this).hasClass("btn-action")) {
      await manageStatus({ status: statusText, id: userId }, $(this));
    }
  ...
  async function fetchUsers(payload) {
    try {
      const result = await makeRequest(
        `/user/role/${role}/page/${payload.page}`,
        "GET",
        {}
      );
  ...
  async function manageStatus(payload, el) {
    ...
    if (confirm(`Are you sure to ${payload.status} account?`)) {
      ...
      try {
        const result = await makeRequest(
          `/user/${payload.id}/status/${payload.status}`,
          "PUT",
          {},
        );

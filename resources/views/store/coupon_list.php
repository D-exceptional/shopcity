...
<body class="hold-transition sidebar-mini">
        ...
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-sm-6 header-count">
              <h1><b>Coupons (<?= $numberManager->format($couponList['total']) ?>)</b></h1>
            </div>
            <div class="col-sm-6">
                ...
                <?php if (!empty($couponList['coupons'])): ?>
                  ...
                  <?php foreach ($couponList['coupons'] as $coupon): ?>
                    ...
                    <tr class='content-row' data-id='<?= $coupon['coupon_id'] ?>' data-status='<?= $coupon['coupon_status'] ?>'>
                      ...
                      <td class='coupon-discount'><?= $numberManager->format((int)$coupon['coupon_discount'] ?? 0); ?></td>
                      <td class='coupon-status'>
                        <select class='form-control' disabled>
                          <option value='<?= $coupon['coupon_status'] ?>'><?= $coupon['coupon_status'] ?></option>
                          <option value='<?= $coupon['coupon_status'] === 'Active' ? 'Deactivated' : 'Active' ?>'>
                            <?= $coupon['coupon_status'] === 'Active' ? 'Deactivated' : 'Active' ?>
                          </option>
                        </select>
                      </td>
  ...
  <!-- Custom Scripts -->
  <script src="<?= asset_versioned('js/store/coupon-list.js'); ?>" type="module"></script>
